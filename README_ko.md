...
- **🎨 스마트 로고 오버레이**: 로고 이미지를 업로드하면 워터마크 위치에 автоматчески 배치됩니다:
  - **자동 위치**: 로고가 감지된 워터마크 위치에 자동으로 배치됩니다
  - **일관된 크기**: 로고 크기는 이미지의 짧은 변을 기준으로 계산됩니다 (가로/세로 같은 크기)
  - **調整 가능**: 투명도 0%~100% (기본값 20%), 크기 10%~300% (기본값 200%)
- **🔧 출력 옵션**:
...
- **📥 유연한 다운로드 옵션**:
  - **단일 이미지**: 다운로드 버튼을 클릭하여 로고 버전 가져오기 (R_/S_ 접두사). «N» 체크박스로 로고 없음 버전 다운로드 가능.
  - **일괄 다운로드**: 로고를 업로드하면 ZIP에 두 버전이 포함됩니다:
    - 로고 포함: `R_이미지_clean.png` (가로) / `S_이미지_clean.png` (세로)
    - 로고 없음: `N_이미지_clean.png`
